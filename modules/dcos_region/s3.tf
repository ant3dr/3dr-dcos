...
resource "aws_s3_bucket" "bootstrap_bucket" {
  ...
  # use a custom policy so that HTTP urls work inside the bucket to fetch bootsraps!
  policy = <<EOF
{
    "Version": "2012-10-17",
    ...
    "Statement": [
    {
        ...
        "Effect": "Allow",
        "Resource": ["arn:aws:s3:::dcos-bootstrap-${var.aws_region}","arn:aws:s3:::dcos-bootstrap-${var.aws_region}/*"],
        "Condition": {
          "StringEquals": {
...
# just assume usual IAM roles for this access
