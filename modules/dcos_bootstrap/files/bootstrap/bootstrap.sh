...
# check for the existence of the tools we need
hash confd 2>/dev/null || die "require confd"
hash docker 2>/dev/null || die "require docker"

if [ "$#" -ne 6 ]; then
  die "Usage: ./bootstrap.sh cluster_name exhibitor_s3_bucket exhibitor_prefix exhibitor_address bootstrap_url s3_output_loc"
fi 
